                ...
                <div className="flex items-center gap-4 relative z-10">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                            ...
                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none">Live Help Active</span>
                        </div>
                    </div>
                </div>
                <div className="relative z-10">
                    <span className={`px-4 py-1.5 rounded-xl border-2 text-[10px] font-black uppercase tracking-widest shadow-sm ${
                        ...
                        <div key={msg.id} className={`flex ${isAdmin ? 'justify-start' : 'justify-end'}`}>
                            <div className={`flex flex-col max-w-[85%] ${isAdmin ? 'items-start' : 'items-end'}`}>
                                <div className={`flex items-center gap-3 mb-2 px-2`}>
                                    ...
                                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{format(new Date(msg.created_at), 'hh:mm a')}</span>
                                </div>
                                <div className={`relative px-5 py-4 rounded-[1.5rem] shadow-md border ${
                                    isAdmin 
            ...
            {ticket?.status !== 'closed' ? (
                <div className="p-6 bg-slate-50 border-t border-gray-100">
                    ...
                    <form onSubmit={handleSend} className="flex items-center gap-3 bg-white p-2 rounded-[1.5rem] border border-gray-200 shadow-sm focus-within:border-slate-900 transition-all">
                        <label className="p-3 text-gray-400 hover:text-slate-900 cursor-pointer transition-colors hover:bg-slate-50 rounded-2xl">
                ...
                <div className="p-8 bg-slate-100/50 text-center">
                    <p className="text-xs font-black text-slate-400 uppercase tracking-[0.2em]">This conversation has been concluded</p>
                </div>
            )}
        </div>
    );
